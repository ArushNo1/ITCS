...
public class WinterWonderWsnow extends JPanel{
	...
	public void paintComponent (Graphics g)	{
		...
		Snowflake[] ss = new Snowflake[50];
		for(int i = 0; i < ss.length; i++) {
			int x = rand.nextInt(0,800);
			int y = rand.nextInt(0,600);
			int size = rand.nextInt(5,10);
			int r = rand.nextInt(240,255);
			int G = rand.nextInt(240,255);
			int b = rand.nextInt(240,255);
			ss[i] = new Snowflake(x,y,size, new Color(r,G,b));
			ss[i].draw(g);
		}
	...
	public static void main(String[] args) {
		...
		frame.setContentPane(new WinterWonderWsnow());
